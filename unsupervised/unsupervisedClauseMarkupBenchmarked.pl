...
my $host = "localhost";
my $port = 1527;
my $url = "jdbc:derby:biocreative2012;create=true;user='root';password='forda444'";
my $user = "biocreative";
...
$create = $dbh->prepare('create table if not exists '.$prefix.'_sentence (sentid int(11) not null unique, source varchar(500), sentence text, originalsent text, lead varchar(2000), status varchar(20), tag varchar('.$taglength.'),modifier varchar(150), charsegment varchar(500) DEFAULT NULL,type varchar(20), primary key (sentid)) CHARACTER SET utf8 engine=innodb');
